...
const navItems = [
  ...
  { name: 'My Listings', path: '/host/listings', icon: House },
  { name: 'Bookings', path: '/host/bookings', icon: CalendarDays },
  { name: 'Earnings', path: '/host/earnings', icon: Wallet },
  { name: 'Messages', path: '/host/messages', icon: MessageCircle },
...
const HostSidebar = () => {
  ...
  // Determine if a route is active (exact match for overview, startsWith for others)
  const isActive = (path) => {
    if (path === '/host/dashboard') return location.pathname === '/host/dashboard' || location.pathname === '/host';
  ...
  return (
    <div className="w-64 h-full bg-white border-r border-gray-200 flex flex-col justify-between py-6 px-4">
      <div>
        <Link to="/host/dashboard" className="flex items-center gap-2 px-2 mb-10 group">
          ...
          <div>
            ...
            <div className="text-[10px] font-bold text-brand-500 uppercase tracking-widest mt-0.5">Host</div>
          </div>
        </Link>
